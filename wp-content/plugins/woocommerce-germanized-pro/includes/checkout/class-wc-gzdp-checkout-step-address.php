...
class WC_GZDP_Checkout_Step_Address extends WC_GZDP_Checkout_Step {
	...
	public function after_checkout_validation( $posted_data, $errors = false ) {
		...
		if ( ! is_user_logged_in() && ( $registration_required || ! empty( $posted_data['createaccount'] ) ) ) {
			...
			if ( ! empty( $email ) && is_email ( $email ) && email_exists( $email ) ) {
				wc_add_notice( apply_filters( 'woocommerce_germanized_pro_registration_error_email_exists', __( 'An account is already registered with your email address. Please log in.', 'woocommerce-germanized-pro' ), $email ), 'error' );
			}
		}
		...
		if ( 'yes' === get_option( 'woocommerce_gzdp_checkout_privacy_policy_first_step' ) && 'yes' === get_option( 'woocommerce_gzdp_checkout_privacy_policy_checkbox' ) ) {
			if ( ! isset( $_POST['gzdp_privacy_policy'] ) ) {
				wc_add_notice( apply_filters( 'woocommerce_germanized_pro_privacy_policy_error_message', __( 'Please accept our privacy policy to continue.', 'woocommerce-germanized-pro' ) ), 'error' );
			}
		}

		if ( wc_notice_count( 'error' ) != 0 || ( $errors && ! empty( $e_messages ) ) )
			return;
