...
class WC_GZDP_Invoice_Helper {
	...
	public function create_refund( $refund_id, $args ) {
	    ...
	    $p = get_post( wc_gzd_get_crud_data( $refund_order, 'id' ) );
        ...
        $cancellation->refresh( array( 'invoice_parent' => $invoice->id ), $refund_order );

        // Lock parent invoice
        update_post_meta( $invoice->id, '_invoice_locked', 'yes' );

        // Maybe enable redirect
        update_post_meta( wc_gzd_get_crud_data( $order, 'id' ), '_refund_invoice_created', 'yes' );
	...
	public function admin_hooks() {
		...
		add_action( 'woocommerce_gzd_before_save_section_invoices', array( $this, 'check_number_reset' ), 10, 1 );
	...
	public function shop_order_search_custom_fields( $wp ) {
		...
		if ( 'edit.php' != $pagenow || $wp->query_vars['post_type'] != 'shop_order' ) {
			return;
		}
	...
	public function set_order_meta_crud( $item, $cart_item_key, $values, $order ) {
		...
		if ( function_exists( 'wc_gzd_set_crud_meta_data' ) && is_a( $item, 'WC_Order_Item' ) && $item->get_product() ) {
			$product = $item->get_product();

			add_filter( 'pre_option_woocommerce_tax_display_shop', array( $this, 'set_price_exclude' ), 0 );

			$item = wc_gzd_set_crud_meta_data( $item, '_unit_price_excl', wc_gzd_get_gzd_product( $product )->get_unit_html( false ) );
	...
	public function generate_subscription_invoice( $renewal_order, $subscription ) {
		$this->auto_generate_invoice( $renewal_order->id );
	...
	public function auto_generate_invoice( $order_id ) {
		...
		if ( ! apply_filters( 'woocommerce_gzdp_generate_invoice', true, $order ) || ( get_option( 'woocommerce_gzdp_invoice_auto_gateways' ) && ! in_array( wc_gzd_get_crud_data( $order, 'payment_method' ), get_option( 'woocommerce_gzdp_invoice_auto_gateways', array() ) ) ) )
			return;
	...
	public function auto_generate_cancellation( $order_id ) {
		...
		if ( ! empty( $invoices ) ) {
			...
			foreach ( $invoices as $invoice ) {
			    ...
			    // Do not generate full cancellations for invoices that have partial refunds or are already cancelled
				if ( $invoice->is_cancelled() || $invoice->is_partially_refunded() )
					continue;
	...
	public function get_settings() {
		...
		$options = array(
			...
			array( 
				'title' => _x( 'General', 'invoices', 'woocommerce-germanized-pro' ), 
				'type' => 'title', 
				'id' => 'invoice_general_options', 
				'desc' => _x( 'Here you may find settings related to generating invoices & packing slips. Adjust these settings to customize your invoices.', 'invoices', 'woocommerce-germanized-pro' ), 
			),
	...
	public function add_meta_boxes() {
		...
		if ( ! empty( $invoices ) ) {
			...
			foreach ( $invoices as $id => $invoice ) {
				...
				if ( 'simple' === $invoice->type && ! $invoice->is_cancellation() && ! $invoice->is_cancelled() && ! $invoice->is_partially_refunded() ) {
