...
class WC_GZDP_Contract_Helper {
	...
	public function remove_email_payment_instructions( $order, $sent_to_admin, $plain_text, $email ) {
		...
		if ( $email->id === 'customer_processing_order' && wc_gzdp_order_needs_confirmation( wc_gzd_get_crud_data( $order, 'id' ) ) ) {
	...
	public function set_gzd_confirmation_text_hook() {
		...
		add_action( 'woocommerce_update_options_email_customer_order_confirmation', array( $this, 'save_confirmation_text_option' ) );
	}
	...
	public function order_email_actions( $actions ) {
		...
		if ( ! wc_gzdp_order_needs_confirmation( wc_gzd_get_crud_data( $theorder, 'id' ) ) ) {
	...
	public function confirm_order( $order_id ) {
		...
		if ( isset( $gateways[ wc_gzd_get_crud_data( $order, 'payment_method' ) ] ) ) {
			...
			$gateway = $gateways[ wc_gzd_get_crud_data( $order, 'payment_method' ) ];
	...
	public function maybe_disallow_stock_reducing( $is_allowed, $order ) {
		...
		if ( ! get_post_meta( wc_gzd_get_crud_data( $order, 'id' ), '_order_stock_reduced', true ) )
			return $is_allowed;
	...
	public function get_order_payment_url( $order_id ) {
		...
		if ( wc_gzd_get_crud_data( $order, 'order_payment_info' ) )
			return wc_gzd_get_crud_data( $order, 'order_payment_info' );
	...
	public function add_payment_link( $template_name, $template_path, $located, $args ) {
		...
		if ( $show ) {
			add_action( 'woocommerce_thankyou_' . wc_gzd_get_crud_data( $order, 'payment_method' ), array( $this, 'show_payment_link' ), 10, 1 );
		}
	}
	...
	public function admin_order_actions( $actions, $the_order ) {
		if ( wc_gzdp_order_needs_confirmation( $the_order ) ) {
			...
			$actions[ 'complete' ] = array(
				'url'       => $this->get_confirmation_url( wc_gzd_get_crud_data( $the_order, 'id' ) ),
	...
	public function admin_order_table_title( $column ) {
        ...
        if ( empty( $the_order ) || wc_gzd_get_crud_data( $the_order, 'id' ) != $post->ID )
	...
	public function admin_order_confirm_button( $order ) {
		...
		echo '<p class="wc-gzdp-submit-wrapper"><a href="' . $this->get_confirmation_url( wc_gzd_get_crud_data( $order, 'id' ) ) .'" id="wc-gzdp-confirm-order-button" class="button button-primary">' . __( 'Confirm Order', 'woocommerce-germanized-pro' ) . '</a></p>';
	}

	public function admin_resend_order_emails( $emails ) {
		global $theorder;
		if ( isset( $theorder ) && wc_gzdp_order_needs_confirmation( wc_gzd_get_crud_data( $theorder, 'id' ) ) ) {
			return array( 
