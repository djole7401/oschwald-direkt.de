...
class WPBackItUp_Premium_Restore {
   ...
   function __destruct() {
	 ...
	 *
	 */
	public function delete_restore_folders() {
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin delete restore folders.' );

		try {
			$file_system = new WPBackItUp_FileSystem($this->log_name);

			//get a list of all the restore folders
			$folder_list = glob($this->restore_root_folder_path .'/*', GLOB_ONLYDIR);
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Delete restore folders completed.');

			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Delete root files.');
			//get a list of all the file
			$files = glob($this->restore_root_folder_path .'/*');
			$file_system->delete_files($files);
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End Delete root files.');
	...
	public function delete_restore_folder_async() {
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin delete restore folders.' );
        ...
        global $WPBackitup;
        $WPBackitup->dispatch_async_tasks(Processors::DIRECTORY_CLEANUP, $item_list);
	...
	public function delete_staged_folders() {
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin delete staged folders.' );
	...
	private function delete_folders($item_list) {
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Folders to be deleted:' );
		WPBackItUp_Logger::log($this->log_name,$item_list);

		$file_system = new WPBackItUp_FileSystem($this->log_name);
		foreach($item_list as $item) {
			if (is_dir($item)) {
				if (! $file_system->recursive_delete( $item )){
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Folder could NOT be deleted:' . $item);
					return false;
				}
			}else{
				if (! unlink($item)){
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'File could NOT be deleted:' . $item);
					return false;
				}
			}
		}
	...
	public function create_restore_root_folder() {
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Create restore folder.' . $this->restore_root_folder_path);
	...
	function unzip_archive_file($backup_set_list){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin -  Unzip Backup File:' .$backup_file_path);
		try {
			$zip = new ZipArchive;
			...
			if ($res === TRUE) {
				if (true===$zip->extractTo($this->restore_root_folder_path)){
					...
					$zip->close();
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant unzip backup:'.$backup_file_path);
					return false;
				}
			} else {
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant open backup archive:'.$backup_file_path);
				return false;
			}

			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Backup file unzipped: ' .$backup_file_path);

		} catch(Exception $e) {
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'An Unexpected Error has happened: ' .$e);
			return false;
		}
	...
	function validate_restore_folder($current_job){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Validate restore folder : ' .$restore_folder_root);

		$site_data_folder = $restore_folder_root .self::SITEDATAPATH;
		if(!is_dir($site_data_folder) ){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'site-data missing from restore folder:' .$site_data_folder);
			return false;
		}
		...
		if(true===$current_job->getJobMetaValue( 'restore_plugins') && !is_dir($plugins_folder) ){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'wp-content-plugins missing from restore folder:' .$plugins_folder);
			return false;
		}

		$themes_folder = $restore_folder_root .self::THEMESPATH;
		if(true===$current_job->getJobMetaValue( 'restore_themes') && !is_dir($themes_folder) ){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'wp-content-themes missing from restore folder:' .$themes_folder);
			return false;
		}
		...
		if(true===$current_job->getJobMetaValue( 'restore_others') && !is_dir($other_folder) ){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'wp-content-other missing from restore folder:' .$other_folder);
		}
		...
		if(true===$current_job->getJobMetaValue( 'restore_uploads') && !is_dir($uploads_folder) && !$this->validate_no_uploads()){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'wp-content-uploads missing from restore folder:' .$uploads_folder);
			return false;
		}

		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End - Restoration directory validated: ' .$restore_folder_root);
		return true;
	}
	...
	function validate_siteinfo_file(){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Validate Site info file: ' . $site_info_path);
		if(! file_exists($site_info_path)) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Validate Site info file: ' . $site_info_path);
			if(! file_exists($site_info_path)) {
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'backupsiteinfo.txt missing or empty.');
				return false;
			}else {
				...
				//Get file values
				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'GET Site Info data' );
				$import_siteinfo_lines = file( $site_info_path);
				if (!is_array($import_siteinfo_lines) || count($import_siteinfo_lines)<3){
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Site Data file NOT valid.' );
					return false;
				} else {
					...
					WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Site Data:' );
					WPBackItUp_Logger::log($this->log_name,$site_data);
					return $site_data;
				}
			}
		}
		...
		{
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Site Data file NOT valid:' .var_export($siteinfo_array,true));
			return false;
		}
	...
	function validate_manifest_file($backup_set_list, &$error_code){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Validate backup manifest: ' . $manifest_file_path);
		...
		if (false===$manifest_data_string || empty($manifest_data_string)){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Manifest empty.');
			$error_code=1;
			return false;
		}
		...
		{
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Zip File Missing');
			$error_code=2;
			return false;
		...
		if (count($backup_set_list) > count($manifest_data_array) )
		{
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Extra Zip File.');
			$error_code=3;
			return false;
		}
			...
			if (array_key_exists($file_name,$manifest_data_array)){
				...
				if ($real_file_size!=$manifest_file_size){
					...
					if ( strrpos( $file_name, '-main-' ) !== false ){
						if ($real_file_size<$manifest_file_size) {
							$error_code=4;
							WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Size doesnt match manifest' .$zip_file);
							return false;
						}
					} else {
						$error_code=4;
				...
				$error_code=3;
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Zip File NOT in manifest:' .$zip_file);
				return false;
			}
		}
	...
	function validate_no_uploads(){
		...
		if(! file_exists($manifest_file_path)) {
			...
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Validate backup manifest: ' . $manifest_file_path );
			if(! file_exists($manifest_file_path)) {
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Backup manifest not found: ' . $manifest_file_path );
				return false;
			}
		}
		...
		foreach($manifest_data_array as $filename=>$filesize){
			if ( strrpos( $filename, '-uploads-' ) !== false ){
				WPBackItUp_Logger::log_info($this->log_name, __METHOD__, 'uploads folder exists');
				return false; 
			}
		}
		WPBackItUp_Logger::log_info($this->log_name, __METHOD__, 'uploads folder NOT in Manifest');
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End' );
		return true;
	...
	function validate_SQL_add_items(){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'SQL files found:'. var_export($sql_files,true));
		...
		foreach ($sql_files as $sql_file){
			...
			foreach($exclude_table_list  as $exclude_table_name) {
				...
				if ($pos!==false) { //found
					WPBackItUp_Logger::log_info($this->log_name,__METHOD__, sprintf('EXCLUDE table:%s',$sql_file_name));
			...
			if (false===strpos($sql_file_name, '_options-')) {
				...
				$sql_exists = true;
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'SQL File:' . var_export( $sql_file, true ) );
			} else{
		...
		foreach ($options_files as $option_file){
			...
			$sql .= "(" . $this->backup_id . ", '" . WPBackItUp_Job_Item::DATABASE . "', '" . $sql_file_name . "', " . $file_size . ",'" . current_time( 'mysql' ) . "' ),";
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'SQL File:' . var_export( $option_file, true ) );
		}
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Database backup files exist:' .$sql_exists?'true':'false');
		return $sql_exists;
	}
	...
	public function export_database(){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin - Export Database: ' .$backup_file);

		//Try SQLDump First
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Export DB with MYSQLDUMP');
		if(!$sqlUtil->mysqldump_export($backup_file) ) {

			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Export DB with MYSQLDUMP/PATH');
			if(!$sqlUtil->mysqldump_export($backup_file,true) ) {

				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Export DB with Manual SQL EXPORT');
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Database Exported successfully');
	...
	public function rename_folder($from_folder_name,$to_folder_name){
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin');
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Rename from folder name:' . $from_folder_name);
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Rename to folder name: '. $to_folder_name);
	...
	public function restore_wpcontent(){
		...
		foreach ( $wpcontent_folder_list as $from_folder_name ) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Folder name:' . $folder_name_only);
			...
			if (is_dir($to_folder_name)){
				...
				for ($i = 1; $i <= 5; $i++) {
					WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Archive attempt:' . $i);
			...
			if ($archive_success) {
				for ($i = 1; $i <= 5; $i++) {
					WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Restore attempt:' . $i);
			...
			if (! $rename_success){
				...
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant restore folder.' .$from_folder_name );
			}
		}


		if ( is_array($error_folders) && count($error_folders)>0){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'End - Error Folders:');
			WPBackItUp_Logger::log($this->log_name,$error_folders);
			return $error_folders;
		}

		//WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End Restore WPContent Folders-No errors');
		...
		foreach ( $wpcontent_files_only as $from_file_name ) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'WPContent File name:' . $file_name_only);
			...
			if (!$rename_success){
				...
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant restore file.' .$from_file_name );
		...
		if ( is_array($error_files) && count($error_files)>0) {
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'End - Error Files:');
			WPBackItUp_Logger::log($this->log_name,$error_files);
			return $error_folders;
		} else{
			//WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End Restore WPContent - No errors');
			return true;
		}
	...
	public function restore_plugins(){
		...
		foreach ( $plugins_folder_list as $from_folder_name ) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Plugin Folder name:' . $folder_name_only);
			if ( $folder_name_only != WPBACKITUP__PLUGIN_FOLDER) {
				...
				if (!$rename_success){
					array_push($error_folders,$from_folder_name);
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant restore plugin folder.' .$from_folder_name );
				}
			}
		}
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End restore plugin folders.');
		...
		foreach ( $plugins_files_only as $from_file_name ) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Plugin File name:' . $file_name_only);
			...
			if (!$rename_success){
				array_push($error_files,$from_folder_name);
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Cant restore plugin file.' .$from_file_name );
		...
		if (is_array($error_files) && count($error_files)>0) {
			...
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End Restore Plugins - No errors');
			return true;
		}
	...
	public function restore_database($current_job) {
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Begin -  restore database.' );
		...
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Process SQL File:' . $sql_file_path );

		//run the import
		$run_import = $this->run_sql_from_file( $sql_file_path );
		if (false===$run_import){
			$item->setStatus(WPBackItUp_Job_Item::ERROR);
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Import Error.');
			return false;
		}
		...
		if (false!==strpos($sql_file_name, '_options-') || 0==$remaining_count){
			...
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Dispatch Background Process:'.$current_job->getJobName() );
			...
			if (!empty($stashed_wpbackitup_options) && true===$this->update_create_options($stashed_wpbackitup_options)){
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'WPBackItUp Options Updated' );
			...
			//deactivate plugins in options table except for wpbackitup CE and premium
			if ($this->update_create_option('active_plugins',array("wp-backitup/wp-backitup.php","wp-backitup-premium/wp-backitup-premium.php"),'yes')){
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Plugins deactivate in options table.' );
			}
			...
			if (!empty($stashed_siteurl) && true===$this->update_siteurl_option($stashed_siteurl)){
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Site URL Updated' );
			...
			if (!empty($stashed_homeurl) && $this->update_homeurl_option($stashed_homeurl)){
				WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'HomeURL Updated' );
		...
		if (false!==strpos($sql_file_name, '_users-')  || 0 == $remaining_count) {
			...
			if ( ! empty( $stashed_user ) ) {
				...
				if (! $this->update_create_user($stashed_user)){
					WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'User could not be updated.' );
				}
			...
			if ( ! empty( $stashed_user ) ) {
				...
				if (! empty($current_user)){
					...
					if (! $this->update_create_usermeta($current_user->ID,$stashed_usermeta)){
						WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'One or more meta values were not updated.' );
					}
				}
		...
		if ( 0 == $remaining_count) {
			...
			if ( ! $this->delete_duplicate_users( $current_user->ID, $current_user->user_login ) ) {
				WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'Error removing duplicate profiles' );
			}
	...
	public function rename_snapshot_tables($rename_sql_file) {
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Begin -  Rename snapshot tables:' .$rename_sql_file);

		//run the SQL rename script
		if ( false === $this->run_sql_from_file( $rename_sql_file ) ) {
			WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'SQL Import Error.' );
			return false;
		}
	...
	public function update_snapshot_database($current_job,$snapshot_prefix) {
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Begin -  Update snapshot tables.' );
		...
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Rename Snaphsot prefix SQL File:' . $rename_sql_file );
		if (false===$this->generate_rename_sql($rename_sql_file,$snapshot_prefix)){
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'WPBackItUp Options Updated in snapshot table.' );
			return false;
		}
		...
		if (!empty($stashed_siteurl) && true===$this->update_siteurl_option($stashed_siteurl,$snapshot_prefix)){
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Site URL Updated in snapshot table.' );
		}

		//update homeurl in snapshot table
		$stashed_homeurl = $current_job->getJobMetaValue('current_homeurl');
		if (!empty($stashed_homeurl) && $this->update_homeurl_option($stashed_homeurl,$snapshot_prefix)){
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'HomeURL Updated in snapshot table.' );
		}

		//update upload_path & upload_url_path in snapshot table
		if ($this->update_upload_options($snapshot_prefix)){
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Uploads Options Updated in snapshot table.' );
		}
		...
		if ($this->update_create_option('active_plugins',array("wp-backitup/wp-backitup.php","wp-backitup-premium/wp-backitup-premium.php"),'yes',$snapshot_prefix)){
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Plugins deactivate in snapshot table.' );
		}
		...
		if ( !empty($backup_table_prefix) && ! $this->update_snapshot_table_prefix($snapshot_prefix,$backup_table_prefix)){
			WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'Table Prefix could not be updated.' );
		}
	...
	private function run_sql_from_file($sql_file_path){
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin - SQL: '. $sql_file_path);
		...
		if(!$dbc->run_sql_exec($sql_file_path)) {
			...
			if(!$dbc->run_sql_exec($sql_file_path,true)) {
				...
				if ( ! $dbc->run_sql_manual( $sql_file_path ) ) {
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Database import error.' );
					return false;
				}
			}
		}
		...
		foreach ( $plugins as $plugin => $value ) {
			...
			if (! is_plugin_active($plugin) ) {
				...
				if ( is_wp_error( $result ) ) {
					WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Plugin could NOT be activated:' .$plugin);
				} else{
					WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Plugin activated:' .$plugin);
				}
			}
		}
	...
	public function deactivate_plugins(){
		...
		foreach ($plugins as $plugin) {
			//dont deactivate wp-backitup or premium
			if ('wp-backitup/wp-backitup.php' != $plugin && 'wp-backitup-premium/wp-backitup-premium.php' != $plugin){
				deactivate_plugins($plugin);
				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Plugin Deactivated:' . $plugin);
			}
		}
	...
	public function get_siteurl(){
		...
		if (empty($siteurl)) {
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Siteurl not found');
			return false;
		}

		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End - Siteurl found:' .$siteurl);
		return $siteurl;
	}
	...
	function get_homeurl(){
		...
		if (empty($homeurl)) {
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Homeurl not found.');
			return false;
		}
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End - homeurl found:' . $homeurl);
		return $homeurl;
	}

	//get option value
	function get_option_value($option_name,$table_prefix=null){
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Begin:'. $option_name);
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End - option value found.' );
		return $value;
	}
	...
	public function delete_users_except_current( ) {
		...
		if (false===$users_deleted){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Delete user error');
			return false;
		}
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'users deleted: '.$users_deleted);
	...
	public function delete_usermeta_except_current( ) {
		...
		if (false===$usermeta_deleted){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Delete usermeta error');
			return false;
		}
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'usermeta deleted: '.$usermeta_deleted);
	...
	function update_create_options($options,$table_prefix=null){
		...
		foreach($options as $option){
			if ($db->update_create_option($option->option_name,$option->option_value,$option->autoload,$table_prefix)){
				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Option updated in database:' .$option->option_name);
			}else{
				$rtn_val=false;
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Option NOT updated in database:' .$option->option_name);
			}
		}
	...
	/**
	 *  Create or update option in table using SQL
	 *  - If array is passed into value it will be serialized
	 *
	 * @param      $option_name
	 * @param      $option_value
	 * @param      $option_autoload
	...
	function update_create_option($option_name,$option_value,$option_autoload,$table_prefix=null){
		...
		if (empty( $option_value )){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Option value is empty:' .$option_value);
			return false;
		}
		...
		if (is_array($option_value)){
			...
			if (false== $option_value) return false;
		}

		$db = new WPBackItUp_DataAccess();
		if ($db->update_create_option($option_name,$option_value,$option_autoload,$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Option updated in database:' .$option_name);
			return true;
		}else{
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Option NOT updated in database:' .$option_name);
			return false;
		}
	...
	function get_user_by_id($user_id){
		...
		if (empty( $user_id) || $user_id==0){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'User is not available:' .var_export( $user_id, true));
			return false;
		}
	...
	function get_user_by_login($user_login){
		...
		if (empty( $user_login) ){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'User info is not available:' .var_export( $user_login, true));
			return false;
		}
	...
	function get_usermeta_by_id($user_id){
		...
		if (empty( $user_id) || $user_id==0){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'User meta is not available:' .var_export( $user_id, true));
			return false;
		}
	...
	function update_siteurl_option($siteurl_value,$table_prefix=null){
		...
		if (true===$db->update_create_option('siteurl',$siteurl_value,'yes',$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'SiteURL updated in database:' .$siteurl_value);
			return true;
		}

		WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Error: SiteURL updated failed.');
		return false;
	}
	...
	function update_homeurl_option($homeurl_value,$table_prefix=null){
		...
		if (true===$db->update_create_option('home',$homeurl_value,'yes',$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'HomeURL updated in database:'.$homeurl_value);
			return true;
		}
		WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'HomeURL database update failed..');
		return false;
	...
	function update_upload_options($table_prefix=null){
		...
		if (true===$db->update_option('upload_path','',$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'upload_path updated in database.');
		}

		if (true===$db->update_option('upload_url_path','',$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'upload_url_path updated in database.');
		}
	...
	function update_snapshot_currentuser($current_user,$table_prefix){
		...
		if (empty($current_user)){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Invalid user login:'.var_export( $current_user,true));
			return false;
		}

		$db = new WPBackItUp_DataAccess();
		if (true===$db->update_snapshot_currentuser($current_user,$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__, 'User updated in snapshot database.');
			return true;
		}
		
		WPBackItUp_Logger::log_error($this->log_name,__METHOD__, 'User snapshot database update failed.');
		return false;
	}
	...
	function update_create_user($db_user,$table_prefix=null){
		...
		if (true===$db->update_create_user_by_login($db_user,$table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'User updated in database:'.$db_user->user_login);
			return true;
		}
		WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'User database update failed:'.$db_user->user_login);
		return false;
	}
	...
	function create_current_user($db_user){
		...
		if (true===$db->create_user($db_user)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'User created in database.');
			return true;
		}
		WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'User database create failed.');
		return false;
	}
	...
	function update_snapshot_table_prefix($snapshot_prefix,$backup_table_prefix){
		...
		if (empty($snapshot_prefix)){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Invalid snapshot prefix:'.var_export( $snapshot_prefix,true));
			return false;
		}

		if (empty($backup_table_prefix)){
			WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'Invalid backup table prefix:'.var_export( $backup_table_prefix,true));
			return false;
		}

		$db = new WPBackItUp_DataAccess();
		if (true===$db->update_snapshot_table_prefix($snapshot_prefix,$backup_table_prefix)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__, 'Table Prefix updated in snapshot database.');
			return true;
		}

		WPBackItUp_Logger::log_error($this->log_name,__METHOD__, 'Table Prefix snapshot database update failed.');
		return false;
	}
	...
	function delete_duplicate_users($current_user_Id, $current_user_login){
		...
		foreach ($dup_users as $user){
			...
			if (true===wp_delete_user( $user->ID, $current_user_Id)){
				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,sprintf('User was removed successfully:%s-%s', $user->ID,$user->user_login));
			}else{
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,sprintf('User was NOT removed:%s-%s', $user->ID,$user->user_login));
				$rtn_value=false;
			}
		}
	...
	function update_create_usermeta($user_id,$usermeta){
		...
		foreach($usermeta as $meta){
			if ($db->update_create_usermeta($user_id,$meta->meta_key,$meta->meta_value)){
				WPBackItUp_Logger::log_info($this->log_name,__METHOD__,sprintf('User meta updated in database:%s-%s',$user_id,$meta->meta_key));
			}else{
				$rtn_val=false;
				WPBackItUp_Logger::log_error($this->log_name,__METHOD__,sprintf('User meta NOT updated in database:%s-%s',$user_id,$meta->meta_key));
			}
		}
	...
	function update_license_key($license_key){
		...
		if (true===$dbc->run_sql_command($sql)){
			WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'License Key updated in database:'.$license_key);
			return true;
		}

		WPBackItUp_Logger::log_error($this->log_name,__METHOD__,'License Key was not updated.');
		return false;
	}
	...
	function update_permalinks(){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'End - Permalinks updated.');
		return true;
	}
	...
	public function zip_logs(){
		...
		WPBackItUp_Logger::log_info($this->log_name,__METHOD__,'Copy WP Debug: ' .$wpdebug_file_path);
...
//		$message .='<br/><br/>Checkout '. WPBackItUp_Utility::get_anchor_with_utm('www.wpbackitup.com', '', 'notification+email', $term) .' for info about WP BackItUp and our other products.<br/>';
	...
	public function get_backupset_unpack_list($current_job,$current_task) {
		...
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Zips Found in Backup Folder:' . var_export( $backup_files, true ) );
		...
		if ( false !== $backup_set_key ) {
			...
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Backup Set Found:' . $backup_set_file );
			...
			if ( true === $zip->open( $backup_set_file ) ) {
				...
				WPBackItUp_Logger::log_warning( $this->log_name, __METHOD__, 'Failed to open the archive!' );
			}

		} else{
			WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'No backup set found in folder.' );
		}

		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Missing Archives:' . var_export( $missing_archives, true ) );
	...
	public function unpack_backup_from_backupset( $current_job, $current_task,$backupset_file,$backup_file  ) {
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Unpack:' . var_export( $backup_file, true ) );

		if (empty($backup_file) ) {
			WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'Backup file parm is empty' . $backup_file);
			return false;
		}

		if (empty($backupset_file) ) {
			WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'Backup set parm is empty' . $backup_file);
			return false;
		}


		$target_folder = $this->get_backup_folder_path() .'/';
		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Extract TO:' . $target_folder);

		//copy from the zip to the destination folder
		if ( ! copy("zip://".$backupset_file."#" .'backups/' .$backup_file, $target_folder .$backup_file)){
			WPBackItUp_Logger::log_error( $this->log_name, __METHOD__, 'Failed to unpack backup file:' . $backup_file);
			return false;
		}

		WPBackItUp_Logger::log_info( $this->log_name, __METHOD__, 'Unpacked backup file:' . $backup_file);
		return true;
