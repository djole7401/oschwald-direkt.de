 ...
 */
    ...
    $page_title = sprintf(__('%s Restore', 'wp-backitup'), WPBACKITUP__FRIENDLY_NAME );

    //Get instance of license for later
    ...
    $plupload_config = array(
      ...
      '_wpnonce' => wp_create_nonce(WPBACKITUP__NAMESPACE . '-upload'),
...
echo '<div class="error"><p><strong>';
    printf(__('Error: Backup folder does not exist. Please contact %s for assistance.', 'wp-backitup'), WPBackItUp_Utility::get_anchor_with_utm(__('support','wp-backitup'),'support' ,'restore+error','no+backup+folder'));
    echo '</strong></p></div>';
}
...
if (!$restore_folder_exists) {
    echo '<div class="error"><p><strong>';
    printf(__('Error: Restore folder does not exist. Please contact %s for assistance.', 'wp-backitup'), WPBackItUp_Utility::get_anchor_with_utm(__('support','wp-backitup'),'support' ,'restore+error','no+restore+folder') );
    echo '</strong></p></div>';
}
?>

<script type="text/javascript">var __namespace = '<?php echo WPBACKITUP__NAMESPACE; ?>';</script>
<div class="wrap">
        ...
        if ($backup_job_list !== false)
        {
          ...
          foreach ($backup_job_list as $job)
          {
            ...
            <tr <?php echo $class ?> id="row<?php echo $i; ?>">
                ...
                <td>
                    <?php
                    if ($wpbacktiup_license->is_license_active())
                    {
...
<?php } ?>
    ...
    <div id="status" class="widget">
      ...
      <div class="restore-errors">
		...
		<span class="error9998"><div class='isa_error'><?php _e('Error 9999: WPBackItUp Premium is not installed', 'wp-backitup'); ?>.</div></span>
	    <span class="error9999"><div class='isa_error'><?php _e('Error 9999: WPBackItUp Premium License is not active', 'wp-backitup'); ?>.</div></span>
